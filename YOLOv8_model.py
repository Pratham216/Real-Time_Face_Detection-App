...
Accuracy: ~96-98%
F1 Score: ~0.97
Pros: Very accurate, fast for its accuracy level
Cons: Requires more resources than Haar/DNN
"""
...
model = YOLO('face_yolov8n.pt')  # Make sure to download this model file
...
frame_interval = 2  # Process every 2nd frame (YOLO is faster than MTCNN)
frame_count = 0
stream = WebcamStream()
prev_time = time.time()
last_face_locations = []
...
while True:
    ...
    prev_time = curr_time
    cv2.putText(display_frame, f"FPS: {int(fps)} | Model: YOLOv8 (Acc: 96-98%)", (10, 25), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

    cv2.imshow("Face Recognition (YOLOv8)", display_frame)
